...
export default function MemberItem({ status, body }: Props): ReactElement {
  return status === Status.success
    ? (
      <Layout>
        <Head>
          <title>{Pages.Members.title} | {body.name}</title>
        </Head>
            ...
            <div className="row mt-3">
              ...
              <div className="col-lg-8">
                ...
                <div className="row no-gutters mb-5">
                  <div className="mb-2">
                    {body.roles?.map(role => <div key={role.id} className="mr-2">
                      <Tag>{role.name}</Tag>
                    </div>)}
                  </div>
                </div>
                ...
                <div className="d-lg-none mb-4">
                  <img className="responsive mx-auto" style={{ maxWidth: "350px" }} src={body.photo} alt="Фотография человека" />
                </div>

                {body.birthday && <div className="row mb-3">
                  <div className="col-lg-4 col-xl-3 col-md-6 text"><b>Дата рождния:</b></div>
                  <div className="col-lg-8 col-xl-9 col-md-6 text">{moment(body.birthday).format("LL")}</div>
                </div>}
                {body.education && <div className="row mb-3">
                  <div className="col-lg-4 col-xl-3 col-md-6 text"><b>Место учебы:</b></div>
                  <div className="col-lg-8 col-xl-9 col-md-6 text">{body.education}</div>
                </div>}
                {body.workList?.length > 0 && <div className="row mb-3">
                  <div className="col-lg-4 col-xl-3 col-md-6 text"><b>Место работы:</b></div>
                  <div className="col-lg-8 col-xl-9 col-md-6 text">
                    {body.workList.map((work, index) => <div key={index}>
                      {`${work.name}, ${work.end ? `${work.start} – ${work.end} гг.` : `с ${work.start} г.`}`}
                    </div>)}
                  </div>
                </div>}
                {body.spheresList?.length > 0 && <div className="row mb-3">
                  <div className="col-lg-4 col-xl-3 col-md-6 text"><b>Интересы:</b></div>
                  <div className="col-lg-8 col-xl-9 col-md-6 text">{body.spheresList.map(sphere => sphere.name).join(',')}</div>
                </div>}
                {body.interests && <div className="row mb-3">
                  <div className="col-lg-4 col-xl-3 col-md-6 text"><b>Навыки:</b></div>
                  <div className="col-lg-8 col-xl-9 col-md-6 text">{body.interests}</div>
                </div>}
                {body.achievements && <div className="row mb-3">
                  <div className="col-lg-4 col-xl-3 col-md-6 text"><b>Интересы и достижения:</b></div>
                  <div className="col-lg-8 col-xl-9 col-md-6 text">{body.achievements}</div>
                </div>}
              </div>
    ...
    : (<Layout>
      <div className="container">
        ...
        {status === Status.critical && <p>Не удалось загрузить участника. Обновите страницу или <Link href='/' passHref>
